...
module type MYSQL_TABLE = {
  type t;
  let table: Table.t(t);

  module Field: {
    include MYSQL_FIELD with type table = t;
    module Null: MYSQL_NULL_FIELD with type t('a) := t('a);
  };
};

module MakeMysqlTable = (T: Sequoia.NAMED) : MYSQL_TABLE => {
...
let table = (name): (module MYSQL_TABLE) =>
  (module
   {
     include MakeMysqlTable({
       type t;
       let name = name;
     });
   });
    ...
    (~handover, st, e) => {
      ...
      Base.(
        ...
        | [@implicit_arity] If(b, t, f) =>
          let st1 = fn("IF(", [b], "");
          let st2 = fn(~st=st1, st1.repr ++ ", ", [t, f], ")");
